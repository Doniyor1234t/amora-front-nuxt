...
import { A as AppCallBackModal } from './AppCallBackModal-Q9aKnE66.mjs';
import { z as getRequestURL, e as createError } from '../nitro/nitro.mjs';
import { u as useStrapi, a as useStrapiQuery, b as mapProductFromResponse } from './catalogMappers-DTYkDLs1.mjs';
import { b as useRoute, c as useRuntimeConfig, u as useNuxtApp } from './server.mjs';
import '@iconify/vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><div class="pt-[72px]"><div class="grid grid-cols-2 overflow-hidden min-h-[calc(100vh-72px)] max-lg:grid-cols-[1.1fr_0.9fr] max-md:grid-cols-1"><div class="flex overflow-y-auto no-scrollbar flex-col h-[calc(100vh-72px)] bg-[#fff]"><!--[-->`);
      ssrRenderList(productImages.value, (value, index) => {
        _push(`<div class="relative min-h-[calc(100vh-72px)] flex items-center justify-center"><img${ssrRenderAttr("src", value)} alt="Product image" class="h-full w-full object-cover object-top max-xl:object-contain"></div>`);
      });
      _push(`<!--]--></div><div class="h-[calc(100vh-72px)] overflow-y-auto px-[72px] py-[80px] bg-white max-xl:px-12 max-lg:px-10 max-md:px-8 max-sm:px-4 max-sm:py-8">`);
      if (unref(isProductLoading)) {
        ...
        _push(`<div class="flex flex-col gap-8 w-full max-w-[620px] text-[#0F0F0F]"><div class="flex justify-end"><button type="button" class="h-12 w-12 rounded-full border border-[#E4E0DA] flex items-center justify-center hover:bg-[#F7F5F0] transition-colors" aria-label="Добавить в избранное">`);
        _push(ssrRenderComponent(_component_Icon, {
          name: "app-icon:heart-outlined",
          color: "#0F0F0F",
          size: "22"
        }, null, _parent));
        _push(`</button></div><div class="flex flex-col gap-3"><span class="text-xs tracking-[0.4em] text-[#C16371] uppercase flex items-center gap-3">${ssrInterpolate(collectionName.value)} <span class="w-1 h-1 rounded-full bg-[#C16371]"></span></span><h1 class="text-[72px] font-[masvol] leading-[1] max-lg:text-[56px] max-md:text-[46px] max-sm:text-[36px]">${ssrInterpolate(product.value.name)}</h1><p class="font-semibold text-[26px] max-sm:text-[20px]">${ssrInterpolate(formatPrice(product.value.price, product.value.currency ? product.value.currency : void 0))}</p></div><div class="flex flex-col gap-4"><div class="flex items-center justify-between"><p class="text-xs uppercase tracking-[0.6em] text-[#3D3D3D]">Размер</p>`);
        if (selectedSize.value) {
          _push(`<span class="text-xs uppercase tracking-[0.3em] text-[#C16371]">${ssrInterpolate(selectedSize.value ? selectedSize.value.toUpperCase() : "")}</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        if (hasSizeOptions.value) {
          ...
          ssrRenderList(sizeOptions.value, (size) => {
            ...
            ], "px-6 h-[44px] rounded-full border text-[13px] uppercase tracking-[0.25em] transition-all"])}">${ssrInterpolate(size.toUpperCase())}</button>`);
          });
          _push(`<!--]--></div>`);
        } else {
          _push(`<p class="text-[15px] uppercase tracking-[0.2em] text-[#CDC6BF]">${ssrInterpolate(availableSizesText.value)}</p>`);
        }
        _push(`</div>`);
        ...
        _push(`<div class="flex flex-col border-t border-b border-[#E8E2DC] divide-y divide-[#E8E2DC]"><div class="py-6"><button type="button" class="w-full flex items-center justify-between"><p class="text-xs uppercase tracking-[0.6em] text-[#3D3D3D]">Состав</p>`);
        _push(ssrRenderComponent(_component_Icon, {
        ...
        if (detailsSections.value.composition) {
          _push(`<div class="mt-4"><ul class="flex flex-col gap-1 uppercase text-[#3D3D3D] text-[15px]"><!--[-->`);
        ...
        _push(`</div><div class="py-6"><button type="button" class="w-full flex items-center justify-between"><p class="text-xs uppercase tracking-[0.6em] text-[#3D3D3D]">Уход</p>`);
        _push(ssrRenderComponent(_component_Icon, {
        ...
        _push(`</div><div class="py-6"><button type="button" class="w-full flex items-center justify-between"><p class="text-xs uppercase tracking-[0.6em] text-[#3D3D3D]">Описание</p>`);
        _push(ssrRenderComponent(_component_Icon, {
        ...
        if (detailsSections.value.description) {
          _push(`<div class="mt-4"><p class="text-[15px] leading-relaxed tracking-[0.05em] text-[#3D3D3D]">${ssrInterpolate(descriptionText.value)}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div></div>`);
...
//# sourceMappingURL=_id_-Cjp9oUVA.mjs.map
