...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const composition = computed(() => {
      const fabric = product.value?.fabric ? `${product.value.fabric}` : void 0;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "pt-[72px]" }, _attrs))}><div class="grid grid-cols-2 max-sm:grid-cols-1 min-h-[calc(100vh-72px)]"><div class="relative"><img${ssrRenderAttr("src", productImage.value)} alt="Product image" class="w-full h-full object-cover object-top"><div class="absolute top-0 left-0 w-full h-full bg-black/40"></div><div class="absolute bottom-6 left-6 bg-white/80 px-5 py-3 rounded-full text-xs tracking-[0.2em] uppercase text-[#3D3D3D]">${ssrInterpolate(collectionName.value)}</div></div><div class="pt-[100px] pl-[40px] pr-[40px] max-sm:px-4 max-sm:pt-[32px] overflow-y-auto">`);
      if (unref(isProductLoading)) {
        ...
        _push(`<div class="flex flex-col gap-4 w-full max-w-[600px] text-[#0F0F0F]"><div class="flex flex-col gap-2"><span class="text-xs tracking-[0.2em] text-[#C16371] uppercase">${ssrInterpolate(collectionName.value)}</span><h1 class="text-[64px] font-[masvol] leading-[1.1] max-sm:text-[38px]">${ssrInterpolate(product.value.name)}</h1><p class="font-bold text-[20px] max-sm:text-[18px]">${ssrInterpolate(formatPrice(product.value.price, product.value.currency ? product.value.currency : void 0))}</p></div><div class="flex flex-col gap-2 mt-2"><p class="text-xs tracking-[0.2em] text-[#3D3D3D]">Описание</p><p class="text-sm leading-relaxed">${ssrInterpolate(descriptionText.value)}</p></div><div class="flex flex-col gap-3 mt-4 text-sm"><p class="text-xs tracking-[0.2em] text-[#3D3D3D] uppercase"> Состав и уход </p><ul class="flex flex-col gap-2 uppercase"><!--[-->`);
        ssrRenderList(composition.value, (item, index) => {
          _push(`<li>${ssrInterpolate(item)}</li>`);
        });
        _push(`<!--]--></ul></div><div class="grid grid-cols-2 gap-4 text-sm uppercase max-sm:grid-cols-1"><div class="flex flex-col gap-2"><span class="text-xs tracking-[0.2em] text-[#3D3D3D]"> Размеры </span><p>${ssrInterpolate(availableSizesText.value)}</p></div><div class="flex flex-col gap-2"><span class="text-xs tracking-[0.2em] text-[#3D3D3D]"> Посадка </span><p>${ssrInterpolate(fitText.value)}</p></div></div><div class="w-full grid grid-cols-2 gap-4 mt-[32px] max-sm:grid-cols-1 max-sm:mb-10">`);
...
//# sourceMappingURL=_id_-Dl7e-xRT.mjs.map
